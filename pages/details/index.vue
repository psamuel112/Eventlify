  ...
  <div class="container mt-8">
    ...
    <div class="form_container px-8 py-8">
      ...
      <div>
        ...
        <ImageUpload @upload-image="handleImageUpload" />
      </div>
      <span class="error-message" v-if="imagesErrors.length">{{ imagesErrors[0] }}</span>
    </div>
    ...
    <div class="form_container mt-8 px-8  pt-8">
      ...
      <div class="editor">
        <v-textarea :error-messages="descriptionErrors" v-model="form.description" variant="outlined"
          label="Write a brief message"></v-textarea>
      </div>
    ...
    <div class="form_container mt-8 px-8 py-4 pt-8">
      ...
      <div class="">
        <v-textarea :error-messages="additional_infoErrors" v-model="form.additional_info" variant="outlined"
          label="Write a brief message"></v-textarea>
      </div>
    </div>
...
function validateForm() {
  ...
  if (!form.images || form.images.length === 0) {
    imagesErrors.value.push('image(s) is required');
    valid = false;
  }
  return valid;
}
...
function handleImageUpload(uploadedImages) {
  form.images = uploadedImages.map((url, index) => ({
    url,        // Image URL
    order: index + 1, // Order based on the image's index in the array
  }));
}
</script>
